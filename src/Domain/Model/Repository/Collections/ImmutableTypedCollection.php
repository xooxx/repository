 ...
 */
namespace Xooxx\Foundation\Domain\Model\Repository\Collections;
...
class ImmutableTypedCollection extends SplFixedArray
{
    ...
    {
        if ($type !== $this->type) {
            throw new RuntimeException(sprintf('Provided value must be of type %s, but variable of type %s was given instead.', $this->type, $type));
        }
    }
    /**
     * @param $newval
    ...
    protected function objectGuard($newval)
    {
        ...
        if ($newValClass !== $this->type) {
            throw new RuntimeException(sprintf('Provided value must be instance of %s, but instance of %s was given instead.', $this->type, $newValClass));
        }
    }
}